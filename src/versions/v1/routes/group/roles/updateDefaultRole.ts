...
export default {
    ...
    handler: async (req, res) => {
        try {
            ...
            if (!groupId || !roleId) return res.status(400).json({
                success: false,
            ...
            const role = await prisma.groups.update({
                where: {
                    UUID: groupId
                },
                data: {
            ...
            return res.status(200).json({success: true, data: role.defaultRoleId})
